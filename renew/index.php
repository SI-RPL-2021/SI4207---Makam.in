...
$invoice = mysqli_query($mysqli, "SELECT * FROM transaksi WHERE id_user='$id_user' AND id_renew='' AND verif='yes'");
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse justify-content-end" id="navbarNav">
                <ul class="nav nav-pills">
                    ...
                    <li class="nav-item">
                        <a class="nav-link activepage link-navbar tebel-sedang" href="#">Perpanjang &nbsp;&nbsp;</a>
                    </li>
                    <li class="nav-item">
    ...
    <div class="container">
      <table border="1" width="100%" style="margin-top: 13%;">
        <tr>
            ...
            <th colspan="2" style="text-align: center;border-left: 1px solid black;">Tindakan</th>
        </tr>
        <?php
        ...
        <tr>
          ...
          if ($view['tipe']=="lahan") {
          echo $view['nama']." | ";
          $id_transaksis = $view['id_transaksi'];
          ...
          <td style="text-align: center;border-left: 1px solid black;border-top: 1px solid black;"><?=rupiah($view['total'])?></td>
          <td colspan="2" style="text-align: center;border-top: 1px solid black;border-left: 1px solid black;"><a href="../assets/renew-proses/?transaksi=<?=generatekode();?>&user=<?=$id_user?>&sewa=<?=$view['id_sewa']?>&renew=<?=generatekode();?>&verif=<?=$view['verif']?>&paid=wait&tipe=<?=$view['tipe']?>&nama=<?=$view['nama']?>&email=<?=$view['email']?>&telp=<?=$view['telp']?>&unit=<?=$view['unit']?>&deskripsi=renew%20<?=$view['tipe']?>&total=<?=$view['total']?>&pajak=<?=$view['pajak']?>&metode_bayar=<?=$view['metode_bayar']?>&resi_transfer=<?=$view['resi_transfer']?>">Perpanjang</a></td>
        </tr>
        <?php 
        }
        ?>
